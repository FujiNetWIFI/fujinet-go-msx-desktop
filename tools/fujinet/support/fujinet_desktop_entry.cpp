/*
 * In-process desktop entry for the FujiNet MSX (RS232 target) runtime
 * (libfujinet.so).
 *
 * Ported unchanged from fujinet-go-coco-desktop's copy of this file (the
 * entry wrapper is ~95% target-independent per PORTING.md) except the
 * default web admin bind port, 64003 -- this family's dedicated port for
 * MSX (ADAM 65214, Apple II 64001, CoCo 64002). The SAM speech-audio
 * plumbing below is dead code on this target (SAM is an Atari-hardware
 * add-on the RS232 target has no equivalent of) but is left in place
 * unchanged rather than carrying a divergent copy of this file, matching
 * how the other target-independent patches in this repo are kept verbatim.
 *
